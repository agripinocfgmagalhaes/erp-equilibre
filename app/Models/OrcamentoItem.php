...
class OrcamentoItem extends Model
{
    ...
    {
        return $this->hasMany(OrdenServicoItem::class, 'orcamento_item_id');
    }

    public function quantidadeMedidaAcumulada(): float
    {
        return (float) $this->ordemServicoItens
            ->sum(fn (OrdenServicoItem $osi) => (float) ($osi->medicaoItens->max('quantidade_acumulada') ?? 0));
    }
}
